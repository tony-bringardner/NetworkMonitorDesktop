...
public class NetworkMonitorFrame extends JFrame implements Runnable {
	...
	public void send(String msg, InetAddress address) throws IOException {
		...
		DatagramPacket sendPacket = new DatagramPacket(msg.getBytes(), msg.length(),address, socket.getLocalPort());
	 ...
	 */
	public NetworkMonitorFrame(DatagramSocket sock)  {
		this.socket = sock;
		try {
		...
		setTitle("Network Monitor availible at "+socket.getLocalSocketAddress());
	...
	protected void actionPing() {				
	        String  buf = "ping "+NetworkMonitor.getLocalAddress().toString().substring(1)+" "+socket.getLocalPort();
	...
	protected void actionChangePort() {
		...
		DatagramSocket s = d.showDialog("",socket);
